...
{
	// Wholesave, I hope it works as expected
	s.StreamArray<int16_t>((int16_t*)&shared, sizeof(shared) / 2);
	s.Stream(mSr, mSrLowByteAccess, mDr, mFsmMajorState, mCommand, mDataCounter, mFreeze);
	s.StreamArray<int16_t>(mReadBuffer, sizeof(mReadBuffer) / 2);
	s.StreamArray<int16_t>(mWriteBuffer, sizeof(mWriteBuffer) / 2);
}
